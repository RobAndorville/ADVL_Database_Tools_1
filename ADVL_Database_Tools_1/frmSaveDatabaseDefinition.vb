...
#Region " Form Methods - The main actions performed by this form." '---------------------------------------------------------------------------------------------------------------------------
    ...
    Private Sub btnSaveDatabaseDefintion_Click(sender As Object, e As EventArgs) Handles btnSaveDatabaseDefintion.Click
        ...
        'SaveFileDialog1.Filter = "Database Definition |*.DbDef"
        ...
        Dim myTables As DataTable = Nothing
            ...
            'Dim tableData As New XElement("DatabaseDefinition")
            ...
            databaseData.Add(New XComment("Generated by TDS_Access07Tools application."))
            databaseData.Add(New XComment(""))

            Dim descr = New XElement("Description", txtDatabaseDescr.Text)
            databaseData.Add(descr)

            'databaseData.Add(New XComment("Database description:"))
            'databaseData.Add(New XComment(txtDatabaseDescr.Text))
            databaseData.Add(New XComment(""))
            ...
            'myTables = myConnection.GetSchema()

            'Add database summary:
            Dim summary = New XElement("Summary")
            'summary.Add(New XElement("Database", TDS_AccessTools.DatabasePath))
            ...
            For Each Row In myTables.Rows 'Process each row (each table in the database)
                ...
                Debug.Print("")
                'Debug.Print("Printing columns in table: " & Row.Item("TABLE_NAME").ToString)
                ...
                'For Each myCol In myColumns.Columns
                '    Debug.Print("ColumnName: " & myCol.ColumnName.ToString)
                '    Debug.Print("Caption: " & myCol.Caption.ToString)
                '    Debug.Print("DataType: " & myCol.DataType.ToString)
                '    Debug.Print("AllowDBNull: " & myCol.AllowDBNull.ToString)
                '    Debug.Print("AutoIncrement: " & myCol.AutoIncrement.ToString)
                '    Debug.Print("StringFieldLength: " & myCol.MaxLength.ToString)
                '    Debug.Print("")
                'Next
        ...
        'SaveFilePath = IO.Path.GetDirectoryName(SaveFilePath) & "\" & IO.Path.GetFileNameWithoutExtension(SaveFilePath) & ".DbDef"
